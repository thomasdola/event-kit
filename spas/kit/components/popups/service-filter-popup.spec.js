...
describe('ServiceFilterPopup Component', () => {
    ...
    context('Behaviour', () => {
        ...
        it('calls appropriate method on input change -> from input', () => {
            ...
            const wrapper = setup();
            wrapper.find(`.Filter__Form input[value=${props.filterPriceRange.from}]`).simulate('change', e);
        ...
        it('calls appropriate method on input change -> to input', () => {
            ...
            const e = {target: {value: 100}};
            const wrapper = setup();
            wrapper.find(`.Filter__Form input[value=${props.filterPriceRange.to}]`).simulate('change', e);
            expect(spy).toHaveBeenCalledWith(e);
            spy.restore();            
        });
